...
int main(int argc, char **argv)
{
    ...
    if(argc != 6) {
        ...
        fprintf(stderr, "Input should be <nid> <eid> <vcs> <vppb> <option> for unbind/bind\n");
        return EXIT_FAILURE;
    }
    ...
    struct cxlmi_cmd_fmapi_unbind_vppb_req vppb_req = {vcs, vppb, option};
    printf("**********\n");
    printf("Issuing unbind vppb\n");
    printf("**********\n");
    rc = cxlmi_cmd_fmapi_unbind_vppb(ep, NULL, &vppb_req);
    if(rc != 0) {
        printf("Error unbinding virtual ppb...\n");
        //goto exit_free_ctx;
    }
    goto exit_free_ctx;
